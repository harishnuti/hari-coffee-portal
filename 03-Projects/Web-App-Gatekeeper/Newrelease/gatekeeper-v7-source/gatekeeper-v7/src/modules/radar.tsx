...
import { liveAreaSearch, PlacesError, type LiveResult } from '../services/livesearch';
    ...
    <div class="pane">
      ...
      {customResults.value !== null && !customSearching.value && <>
        ...
        {customResults.value.map((v, i, arr) => {
          const showDivider = i > 0 && arr[i-1].filterEvidence !== null && v.filterEvidence === null;
          return (
          <div key={v.placeId}>
            ...
            <div onClick={() => customOpenId.value = customOpenId.value === v.placeId ? '' : v.placeId}>
              ...
              {debugOpenId.value === v.placeId && (
                <div class="debug-drawer" style="margin-top: .8rem; padding: .8rem; background: var(--bg2); border-radius: 6px; font-size: .85rem; color: var(--faint);" onClick={e => e.stopPropagation()}>
                  ...
                  <div style="display:grid; grid-template-columns: 1fr auto; gap: .2rem; margin-bottom: .8rem;">
                    ...
                    <span>Rating & Volume Bonus</span><span>{((v.scoreBreakdown?.ratingBonus ?? 0) + (v.scoreBreakdown?.depthBonus ?? 0)) >= 0 ? '+' : ''}{(((v.scoreBreakdown?.ratingBonus ?? 0) + (v.scoreBreakdown?.depthBonus ?? 0)) * 100).toFixed(1)}%</span>
              ...
              <div class="badges">
                ...
                {(v.primaryType === 'coffee_shop' || v.primaryType === 'coffee_roastery') && <span class="badge" style="border-color:var(--pass);color:var(--pass)">☕ Coffee Focus</span>}
              </div>
            </div>
