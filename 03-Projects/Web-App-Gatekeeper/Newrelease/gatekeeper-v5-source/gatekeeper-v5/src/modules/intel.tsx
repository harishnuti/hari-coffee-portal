...
export function Intel() {
  ...
  for (const e of E) {
    if (e.Official_Notes && e.Official_Notes !== 'Unknown' && e.Your_Verdict && e.Your_Verdict !== 'Unknown') {
      totalComparisons++;
      const promised = e.Official_Notes.toLowerCase().replace(/[^a-z0-9 ]/g, '').split(/\s+/).filter(w => w.length > 3);
      const tasted = e.Your_Verdict.toLowerCase().replace(/[^a-z0-9 ]/g, '').split(/\s+/).filter(w => w.length > 3);
      
      const overlap = promised.filter(p => tasted.includes(p));
      if (overlap.length >= promised.length * 0.5) exactMatchCount++;
      else if (overlap.length > 0) partialMatchCount++;
    }
  }
  const matchPercent = totalComparisons > 0 ? Math.round(((exactMatchCount + partialMatchCount) / totalComparisons) * 100) : 0;
  ...
  return (
    <div class="pane">
      ...
      <div class="intel-note" style="margin-top:0.5rem; background: var(--bg2); padding: 1rem; border-radius: 6px; border: 1px solid var(--line);">
        <div style="font-size: 2rem; color: var(--pass); font-weight: bold;">{matchPercent}%</div>
