...
async function callSearch(apiKey: string, textQuery: string, signal: AbortSignal): Promise<RawPlace[]> {
...
export async function liveAreaSearch(o: LiveSearchOpts): Promise<LiveSearchOutcome> {
  ...
  try {
    ...
    for (let i = 0; i < queries.length; i += chunk) {
      const batch = queries.slice(i, i + chunk).map((q, idx) => 
        callSearch(o.apiKey, q, controller.signal).then(p => { perKw[i + idx] = p; requestCount++; })
  ...
  if (originLat === null || originLng === null) {
    ...
    originLat = first?.latitude ?? null; originLng = first?.longitude ?? null;
  }

  const out: LiveResult[] = [];
  const evidenceKeywords = [
    "pour over", "pourover", "v60", "filter", "batch brew", "hand brew", "chemex", "origami", "aeropress", "gesha", "geisha", "flat burr", "ek43", "lagom", "timemore", "masl", "varietal", "anaerobic", "honey process", "lactic honey", "natural process", "washed process", "fermentation", "roasted in house", "in-house roast", "brew parameter", "brew ratio"
  ];
  ...
  for (const { raw, matched } of map.values()) {
    ...
    if (MASTER.some(m => m.cafe && (normName.includes(m.cafe.toLowerCase()) || m.cafe.toLowerCase().includes(normName)))) {
