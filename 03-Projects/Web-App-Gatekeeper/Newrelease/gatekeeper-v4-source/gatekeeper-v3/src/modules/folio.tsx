...
export function Folio() {
  ...
  // Group by Cafe
  const map = new Map<string, typeof MASTER>();
  ...
  const cafes = Array.from(map.entries()).sort((a, b) => a[0].localeCompare(b[0]));
              ...
              <div class="debug-drawer" style="margin-top: 1rem; padding: 0; background: transparent;" onClick={e => e.stopPropagation()}>
                {visits.sort((a,b) => b.Date.localeCompare(a.Date)).map(v => (
                  <div class="card" style="margin-bottom: 1rem; border-left: 4px solid var(--pass); background: var(--bg2);">
                    ...
                    {v.Technical_Enrichment && (
                      <div style="font-size: 0.85rem; color: var(--faint); font-style: italic; border-top: 1px dashed var(--line); padding-top: 0.5rem; margin-top: 0.5rem;">
                        <b>Technical Enrichment:</b> {v.Technical_Enrichment}
                      </div>
                    )}
                  </div>
                ))}
